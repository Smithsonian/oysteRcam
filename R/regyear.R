...
#'
#' @description
#' A function that creates a dataframe for a regular year with the following
#' variables:
#'
#' num_reg: The month's two digit sequential number. Ex: January = 01.
#'
#' mon_reg: The name of the month. Ex: October.
#'
#' day_reg: The day in the month. Ex: July 23 = 23.
#'
#' jdate_reg: The Julian date corresponding to a regular year.
#'
#' @return A dataframe for a regular year. The dataframe will be stored as
...
# * Create a function that builds a calendar for a regular year.
# Name the function regyear()
regyear <- function() {
  ...
  day_feb <- c('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', 11:28)
  ...
  # Combine into one vector: day_reg.
  day_reg <- c(day_jan, day_feb, day_mar, day_apr, day_may, day_jun, day_jul,
  ...
  # Combine into one vector: num_reg.
  num_reg <- c(num_jan, num_feb, num_mar, num_apr, num_may, num_jun, num_jul,
  ...
  # Combine into one vector: mon_reg.
  mon_reg <- c(mon_jan, mon_feb, mon_mar, mon_apr, mon_may, mon_jun, mon_jul,
               mon_aug, mon_sep, mon_oct, mon_nov, mon_dec)

  # Define the Julian dates.
  jdate_reg <- c('001', '002', '003', '004', '005', '006', '007', '008', '009', '010',
                 ...
                 '091', '092', '093', '094', '095', '096', '097', '098', '099', 100:365)

  # Combine the four variables into a dataframe, regyear.
  regyear <- data.frame(num_reg, mon_reg, day_reg, jdate_reg)

  # Use assign() to assign regyear as calendar to the global environment.
  assign(x = 'calendar', value = regyear, envir = .GlobalEnv)
